...
async def start(client, message):
    ...
    if type_ != 'shortlink' and settings['shortlink']:
        if not await db.has_premium_access(message.from_user.id):
            ...
            await message.reply(f"[{get_size(files.file_size)}] {files.file_name}\n\nYour file is ready, Ask for premium Here👇\n\n<a href= https://t.me/TeluguMoviesAddaSupport_bot>Admin</a>, Please get using this link. 👍", reply_markup=InlineKeyboardMarkup(btn), protect_content=True)
            return
    else:
        pass
